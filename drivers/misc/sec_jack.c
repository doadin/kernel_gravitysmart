...
#define SEND_KEY_CHECK_TIME_MS	60		/* 50ms */
#define DET_CHECK_TIME_MS	200		/* 200ms */
#define WAKE_LOCK_TIME		(HZ * 5)	/* 5 sec */

#ifdef CONFIG_MACH_COOPER || defined(CONFIG_MACH_GIOATT)
#undef FEATURE_HSSD
#endif
...
static void set_send_key_state(struct sec_jack_info *hi, int state)
{
	...
	if(state != 0)
		...
		if (adc >= 150 && adc < 206){
			pr_info(MODULE_NAME "Return_GT2 ADC VALUE = %d \n", adc);
			return;
		}
		#endif
		if(adc <= 7 || (adc >= 150 && adc < 206))
			{
	...
	hi->send_key_pressed = state;
		...
		else if((adc >= 8 && adc <= 20) || (adc >= 206 && adc < 488))
			{
        ...
        else if((adc >=21 && adc <= 45) || (adc >= 489 && adc < 1150))
			{
...
static void sec_jack_set_type(struct sec_jack_info *hi, int jack_type)
{
	 ...
	 */
	pr_info(MODULE_NAME "SEC_JACK_TYPE  = %d \n", jack_type);
	...
	if (jack_type == SEC_HEADSET_4POLE) {
		...
		if (!hi->send_key_irq_enabled) {
			...
			enable_irq(pdata->send_int_usa);
			enable_irq_wake(pdata->send_int_usa);
#endif
...
#if !defined(CONFIG_MACH_GT2) && !defined(CONFIG_MACH_FLIPBOOK)
			disable_irq(pdata->send_int_usa);
			disable_irq_wake(pdata->send_int_usa);
#endif
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	pr_info("%s : ADC value in probe %d \n", __func__,pdata->get_adc_value);
	for(i= 0 ; i < 3; i++)
...
#if !defined(CONFIG_MACH_GT2) && !defined(CONFIG_MACH_FLIPBOOK)
	ret = request_threaded_irq(pdata->send_int_usa, NULL,
				   sec_jack_send_key_irq_thread,
				   IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING |
				   IRQF_ONESHOT,
				   "sec_headset_send_key", hi);
...
#if !defined(CONFIG_MACH_GT2) && !defined(CONFIG_MACH_FLIPBOOK)
		disable_irq_wake(hi->pdata->send_int_usa);
#endif
   }
	else {
		enable_irq(hi->pdata->send_int);
#if !defined(CONFIG_MACH_GT2) && !defined(CONFIG_MACH_FLIPBOOK)
		enable_irq(hi->pdata->send_int_usa);
#endif
   }
	free_irq(hi->pdata->send_int, hi);
#if !defined(CONFIG_MACH_GT2) && !defined(CONFIG_MACH_FLIPBOOK)
	free_irq(hi->pdata->send_int_usa, hi);
#endif
