...
#define AXIS_RETRY_COUNT 5
#define TRUE 1
#define FALSE 0
bool AXIS_FLAG = FALSE;
extern bool PROX_FLAG;
#endif
...
static void bma_work_func_acc(struct work_struct *work)
{
	...
	if (i >= AXIS_RETRY_COUNT) {
	 pr_err("%s: retry over %d\n", __FUNCTION__, AXIS_RETRY_COUNT);
	 return;
	}

//	printk(KERN_INFO "bma_work_func_acc: PROX_FLAG = %d, AXIS_FLAG = %d\n", PROX_FLAG, AXIS_FLAG);
#endif
