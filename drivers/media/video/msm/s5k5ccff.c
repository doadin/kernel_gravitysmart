...
#define PRE_FLASH 7
#define MOVIE_FLASH 2 //8 SRChoi_110209 changed DATA9 -> DATA3
#define PRE_FLASH_OFF -1
#define FLASH_OFF 0
//shlee_temp Flash --

extern int board_hw_revision; // SRChoi_110209 for GT2 Rev0.2
...
static char mMode = 0;
//shlee_temp Flash ++
static char mFlash_mode = PCAM_FLASH_AUTO;
static char mFlash_exifinfo = 0;
static int mFlash_status = 0;
...
int s5k5ccff_regs_table_init_for_antibanding(void);
...
//NAGSM_Android_SEL_Multimedia_NAMARTA_20110331_START_P110329-2672
		if(mFlash_mode == PCAM_FLASH_ON ||mFlash_mode == PCAM_FLASH_AUTO)
		sensor_metering_control(PCAM_METERING_CENTER);
		else sensor_metering_control(PCAM_METERING_SPOT);
//NAGSM_Android_SEL_Multimedia_NAMARTA_20110331_START_P110329-2672	
		}
		break;
...
//shlee_temp Flash ++
static  int s5k5ccff_set_flash(int lux_val)
{
	int i = 0;

	PCAM_DEBUG("%s, flash set is %d\n", __func__, lux_val);
	printk("%s, flash set is %d\n", __func__, lux_val);		//shlee_temp
	if(mFlash_mode == PCAM_FLASH_OFF)return 0;
	...
	if(lux_val>16)
	{
		S5K5CCFF_WRITE_LIST(s5k5ccff_Main_Flash_On);
		gpio_set_value(CAM_FLASH_FLEN,1);
		mFlash_exifinfo = true;
#ifdef USE_FLASHOFF_TIMER
	...
	{
		/* set to movie mode */
		S5K5CCFF_WRITE_LIST(s5k5ccff_Pre_Flash_On);
		for(i=0;i<lux_val;i++)
		{
			udelay(1);
	...
	mFlash_status = lux_val;
#if 0
	/* setting a sensor #2*/
	if(lux_val==PRE_FLASH_OFF)S5K5CCFF_WRITE_LIST(s5k5ccff_Pre_Flash_Off)
	else if(lux_val==FLASH_OFF && s5k5ccff_status.afcanceled == false)S5K5CCFF_WRITE_LIST(s5k5ccff_Main_Flash_Off)
	else if(lux_val==FLASH_OFF && s5k5ccff_status.afcanceled == true)S5K5CCFF_WRITE_LIST(s5k5ccff_Pre_Flash_Off)
#endif	
	return 0;
}

#ifdef USE_FLASHOFF_TIMER
static void s5k5ccff_flashoff_timer_handler(unsigned long data)
{
	s5k5ccff_set_flash(FLASH_OFF);
}
#endif
...
#if 0     //shlee_temp Flash ++
		case PCAM_FLASH_INFO:
			if(mFlash_mode != PCAM_FLASH_OFF 
				&& mFlash_status != FLASH_OFF 
				&& mFlash_status != PRE_FLASH_OFF)
			{
				s5k5ccff_set_flash(FLASH_OFF);
			}
			PCAM_DEBUG("[S5K5CCFF] PCAM_FLASH_INFO %d \n", mFlash_exifinfo);
			ctrl_info.value_1 = mFlash_exifinfo;
		break;
#endif
		case PCAM_SET_FLASH_MODE:
			printk("%s, flash set is %d, mFlash_mode: %d\n", __func__, ctrl_info.value_1, mFlash_mode); /* SRChoi_110127 for DM cmd - torch */
			
			if(ctrl_info.value_1 != PCAM_FLASH_TURN_ON && ctrl_info.value_1 != PCAM_FLASH_TURN_OFF && ctrl_info.value_1 != PCAM_FLASH_TORCH)
				{
				mFlash_mode = ctrl_info.value_1;
	//NAGSM_Android_SEL_Multimedia_NAMARTA_20110331_START_P110329-2672	
				if(mScene == PCAM_SCENE_BACKLIGHT && mFlash_mode != PCAM_FLASH_OFF)sensor_metering_control(PCAM_METERING_CENTER);
				else if(mScene == PCAM_SCENE_BACKLIGHT && mFlash_mode == PCAM_FLASH_OFF)sensor_metering_control(PCAM_METERING_SPOT);
	//NAGSM_Android_SEL_Multimedia_NAMARTA_20110331_STOP_P110329-2672
				}
			else if(ctrl_info.value_1 == PCAM_FLASH_TURN_ON)
			{
				if(mFlash_mode == PCAM_FLASH_AUTO)
				{
					//s5k5ccff_get_lux(&s5k5ccff_status.current_lux);
					//if(s5k5ccff_status.current_lux < 0x0032)s5k5ccff_set_flash(MOVIE_FLASH);
				}
				else
					s5k5ccff_set_flash(MOVIE_FLASH);
			}
			else if(ctrl_info.value_1 == PCAM_FLASH_TURN_OFF)
			{
				s5k5ccff_set_flash(FLASH_OFF);
			}
			else if(ctrl_info.value_1 == PCAM_FLASH_TORCH)
			{
				mFlash_mode = ctrl_info.value_1;
				s5k5ccff_set_flash(MOVIE_FLASH);
			}
...
void cam_pw(int status)
{
	...
	if(status == 1) //POWER ON
	{
		...
		if( board_hw_revision >= 2 ) //SRChoi_110209 for GT2 Rev0.2
		{
		...
		{
			vreg_set_level(vreg_cam_out9_vdda,  OUT2800mV);		// VDDA 2.8V
			vreg_set_level(vreg_cam_out10_vddreg, OUT1800mV);	// VDD_REG 1.6V		
			vreg_set_level(vreg_cam_out8_vddio,  OUT2800mV);		// VDDIO 2.8V		
		}
	...
	else //POWER OFF
	{
		...
		s5k5ccff_set_flash(FLASH_OFF); //shlee_temp Flash 
	}
...
//	unsigned int	before_time, after_time, i;//I2C SPEED TEST
	...
	if(id != 0x05CC)
	{
		printk("<=PCAM=> [GT2] WRONG SENSOR FW => id 0x%x \n", id);
		rc = -1; // SRChoi_110131 for checking camera module 
	}
	else
		printk("<=PCAM=> [GT2] CURRENT SENSOR FW => id 0x%x \n", id);
...
static long s5k5ccff_set_sensor_mode(int mode)
{
	...
	if(first_start_camera)
	{
		...
		//before_time = get_jiffies_64();			//shlee_temp
		...
		#if 1 //shlee_temp #if defined(CONFIG_SAMSUNG_LTN_COMMON)
		{
			...
			rc = s5k5ccff_regs_table_init_for_antibanding();	
			if (rc < 0) {
				printk("s5k5ccff_sensor_init failed!\n");
			}		
		}
		#endif //CONFIG_SAMSUNG_LTN_COMMON
		//hmin84.park 101202
...
*/
	...
	case SENSOR_PREVIEW_MODE:
	{
		...
		*/
		...
		if(mScene != PCAM_SCENE_OFF) // SRChoi_110507 added delay because of no frame
		{
			printk("No scene off - added 250ms preview delay \n");
			msleep(250); // 250ms
			
			for(cnt=0; cnt<200; cnt++) // 1200
			{
			...
			if (cnt == 200) {
				printk("<=PCAM=> Can't get the vsync - fail!, scene = %d\n", mScene);				
				return -1;
			}
		...
		//shlee_temp Flash ++
		printk(" preview - mFlash_mode = %d \n", mFlash_mode); /* SRChoi_110127 for DM cmd - torch */	
		mFlash_exifinfo = false;
		mFlash_status = FLASH_OFF;
		s5k5ccff_set_flash(FLASH_OFF);
		//shlee_temp Flash --
	}
	...
	case SENSOR_SNAPSHOT_MODE:
	{
		...
		printk("<PCAM> mFlash_mode = %d \n", mFlash_mode);		
	
		if(cur_lux > 0xFFFE) // SRChoi_110420 added flash snapshot
		{
			PCAM_DEBUG("HighLight Snapshot!\n");
			printk("HighLight Snapshot!\n");	

			if (mFlash_mode == PCAM_FLASH_ON)		//shlee_temp Flash 
			{
				s5k5ccff_set_flash(FULL_FLASH);
				S5K5CCFF_WRITE_LIST(s5k5ccff_flash_high_snapshot);		
			}
			else
			{
		...
		else if(cur_lux < 0x0020)
		{
			...
			else
			{
				...
				if (mFlash_mode == PCAM_FLASH_ON || mFlash_mode == PCAM_FLASH_AUTO)		//shlee_temp Flash 
				{
					s5k5ccff_set_flash(FULL_FLASH);
					S5K5CCFF_WRITE_LIST(s5k5ccff_flash_lowlight_snapshot);
				}
				else
				{
		...
		else
		{
			...
			{
				s5k5ccff_set_flash(FULL_FLASH);
				S5K5CCFF_WRITE_LIST(s5k5ccff_flash_snapshot);	
			}
			else
			{
		...
		if(mScene != PCAM_SCENE_OFF) // SRChoi_110507 added delay because of no frame
		{
			printk("No scene off - added 250ms snapshot delay \n");
			msleep(250); // 250ms
		}
	}
	break;
...
static int s5k5ccff_sensor_init_probe(const struct msm_camera_sensor_info *data)
{
	...
	S5K5CCFF_WRITE_LIST(s5k5ccff_pre_init0);
	#endif
...
//latin_cam : support anti-banding
#if 1 //shlee_temp #if defined(CONFIG_SAMSUNG_LTN_COMMON)
cam_antibanding_setting s5k5ccff_antibanding = CAM_ANTIBANDING_60HZ;


int s5k5ccff_get_anti_banding(void)
{
	return s5k5ccff_antibanding;
}

int s5k5ccff_reg_init_60hz(void)
{
	printk("Init Table for Anti-banding 60 Hz\n");
	S5K5CCFF_WRITE_LIST(s5k5ccff_init_60hz);
}

int s5k5ccff_regs_table_init_for_antibanding(void)
{
	int rc;
	printk("s5k5ccff_regs_table_init_for_antibanding() : anti-banding ==> %d Hz\n", s5k5ccff_get_anti_banding());

	if (CAM_ANTIBANDING_60HZ == s5k5ccff_get_anti_banding())
		rc = s5k5ccff_reg_init_60hz();
	return rc;
}

ssize_t s5k5ccff_antibanding_show (struct device *dev, struct device_attribute *attr, char *buf)
{
	int count;

	count = sprintf(buf, "%d", s5k5ccff_antibanding);
	printk("The value of s5k5ccff_antibanding is %d\n", s5k5ccff_antibanding);
	return count;
}

ssize_t s5k5ccff_antibanding_store (struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int tmp = 0;
	...
	if ((CAM_ANTIBANDING_50HZ == (cam_antibanding_setting) tmp)
		...
		s5k5ccff_antibanding = (cam_antibanding_setting) tmp;
		printk("The s5k5ccff_antibanding is set to %d\n",s5k5ccff_antibanding);
	}

	return count;
}

static struct device_attribute s5k5ccff_antibanding_attr = {
	.attr = {
		.name = "anti-banding",
		.mode = (S_IRUGO | S_IWUGO)},
	.show = s5k5ccff_antibanding_show,
	.store = s5k5ccff_antibanding_store
};
#endif //CONFIG_SAMSUNG_LTN_COMMON
//hmin84.park 101202
...
int s5k5ccff_sensor_release(void)
{
	...
	mDTP = 0;
	mInit = 0;
	mFlash_status = 0;      //shlee_temp Flash
...
#if 0 //#ifdef USE_FLASHOFF_TIMER       //shlee_temp Flash 
	init_timer(&flashoff_timer);
	flashoff_timer.function = s5k5ccff_flashoff_timer_handler;
...
static int s5k5ccff_sensor_probe(const struct msm_camera_sensor_info *info,
				struct msm_sensor_ctrl *s) // SRChoi_110209 fixed for sensor init during boot-up
{
	printk("<=PCAM=> s5k5ccff_sensor_probe()"); 
...
#if 1//shlee_temp #if defined(CONFIG_SAMSUNG_LTN_COMMON)
	int rv;
	rv = device_create_file(&pdev->dev,&s5k5ccff_antibanding_attr);
	if(rv)	
		printk("<=PCAM=> s5k5ccff_probe() : device_create_file() is failed with %d", rv);
