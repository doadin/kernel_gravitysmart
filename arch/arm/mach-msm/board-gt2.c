...
#define	BT_nRST		77
#define WLAN_RST  82 //wlan _setup_pascal
#define WLAN_BT_EN  88 //wlan _setup_pascal
#define GPIO_WLAN_HOST_WAKE 17 //wlan _setup_pascal
...
//	GPIO_CFG(GPIO_OUT_129, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),
...
static int bluetooth_power(int on)
{
	...
	if (on) 
	{
		...
		if (gpio_get_value(WLAN_RST) == 0) {
			printk("[BT] wifi off state, WLAN_BT_EN=%d, 2.6v enable\n", gpio_get_value(WLAN_BT_EN));
			//bluetooth_setup_power(1);
			gpio_set_value(WLAN_BT_EN,1);
			printk( "[BT] GPIO_WLAN_BT_EN : high == %d\n", gpio_get_value(WLAN_BT_EN));	
		}
		...
		printk( "[BT] GPIO_BT_nRST : high == %d\n", gpio_get_value(BT_nRST));
		msleep(100);
	...
	else 
	{
		...
		gpio_set_value(BT_nRST, 0);

		if(gpio_get_value(WLAN_RST) == 0) {
			gpio_set_value(WLAN_BT_EN, 0);
			printk("[BT] wifi off state, WLAN_BT_EN=%d, 2.6v disable\n", gpio_get_value(WLAN_BT_EN));
...
#endif
#ifdef CONFIG_S5K5CCFF //PGH			//shlee_temp
	{
...
static struct i2c_board_info touch_i2c_devices[] = {
	...
	#endif
	#if defined CONFIG_TOUCHSCREEN_SAIN // KMJ_DK12
	{
...
#endif	
#ifdef CONFIG_S5K5CA //PGH		//shlee_temp 101119
	{
		I2C_BOARD_INFO("s5k5ca", 0x5A>>1),				//shlee_temp 78 -> 5A
	},
#endif
#ifdef CONFIG_S5K5CCFF //PGH		//shlee_temp
	{
		I2C_BOARD_INFO("s5k5ccff", 0x5A>>1),				//shlee_temp 78 -> 5A
	},
#endif
...
#if 1//PCAM // SRChoi_110209 fixed for sensor init during boot-up
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k5ca_data = {
        .sensor_name    = "s5k5ca",
        .sensor_reset   = 0, 
        .sensor_pwd     = 107, 				//shlee_temp 101119
        .vcm_pwd        = 0, 
...
static struct platform_device *devices[] __initdata = {
	...
	&cam_i2c_gpio_device,        //shlee_temp 
	&smc91x_device,
...
#endif
#ifdef CONFIG_S5K5CCFF //PCAM	//shlee_temp
...
static int wlan_host_wake_init(void)
{
	...
	if (gpio_direction_input (17) < 0)
		printk (KERN_ERR "Unable to configure wlan host wake pin\n");
	return 0;
...
#if 0 // moo : ldo18, ldo19 are already used at USB, Touch (CONFIG_BOARD_REVISION == 1)
...
#if 0 // moo : work for GT2_REV01 (CONFIG_BOARD_REVISION == 1)	
		vreg_set_level(vwlan_3_3v_2, OUT3300mV);
		vreg_enable(vwlan_3_3v_2);
#endif
  vreg_set_level(vwlan_1_8v, OUT1800mV);
  vreg_enable(vwlan_1_8v);
#if 1 // moo : work for GT2_REV01 (CONFIG_BOARD_REVISION != 1)	
		gpio_set_value(WLAN_BT_EN, 1);
#endif
#if 0
		if (gpio_get_value(BT_nRST)==0) {
...
#if 1 // defined(CONFIG_MACH_CALLISTO) || defined(CONFIG_MACH_PASCAL)
 else
 {
  /* power off for sleep current */
...
void wlan_setup_power(int on, int detect)
{
	...
	if (on) {
		...
		if(gpio_get_value(BT_nRST) == 0) 	// PASCAL_WIFI_SETUP				
		{					 
			msleep(30);					 
...
#ifndef ATH_POLLING
	mdelay(100);
//	if (detect) {   //NAGSM_WIFI_MOKINYOUNG_20110423: Notify wlan status even when detecting WiFi chip is failed.
		/* Detect card */
		if (wlan_status_notify_cb)
			wlan_status_notify_cb(on, wlan_devid);
		else
			printk(KERN_ERR "WLAN: No notify available\n");
//	} //NAGSM_WIFI_MOKINYOUNG_20110423: Notify wlan status even when detecting WiFi chip is failed.
#endif /* ATH_POLLING */
}
...
static void __init msm7x2x_init_mmc(void)
{
	...
	sdcc_gpio_init();
#if 0	
if (gpio_tlmm_config(GPIO_CFG(WLAN_BT_EN, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE))
	printk (KERN_ERR "Error configuraing WLAN_BT_EN\n");
if (gpio_tlmm_config(GPIO_CFG(WLAN_RST, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE))
	printk (KERN_ERR "Error configuraing WLAN_RST\n");
if (gpio_tlmm_config(GPIO_CFG(GPIO_WLAN_HOST_WAKE, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE))
...
static void __init msm7x2x_init(void)
{
...
#else
     	i2c_register_board_info(4, cam_i2c_devices, ARRAY_SIZE(cam_i2c_devices));    //shlee_temp 0 -> 4
	...
	i2c_register_board_info(3, mus_i2c_devices_new, ARRAY_SIZE(mus_i2c_devices_new));
      ...
      if (board_hw_revision>=0x04)
            platform_device_register(&keypad_device_surf_04);
	if (board_hw_revision>=0x02 && board_hw_revision<0x04)
		platform_device_register(&keypad_device_surf_02);
	if (board_hw_revision<0x02)
		platform_device_register(&keypad_device_surf);
#endif

	 if (board_hw_revision>=0x04)
            keypad_gpio_control(&keypad_device_surf_04);
	if (board_hw_revision>=0x02 && board_hw_revision<0x04)
		keypad_gpio_control(&keypad_device_surf_02);
	if (board_hw_revision<0x02)
...
//	pr_info(" SOCINFO Major:%d, Minor:%d \r\n",SOCINFO_VERSION_MAJOR(socinfo_get_version()),SOCINFO_VERSION_MINOR(socinfo_get_version()) );
		if ((SOCINFO_VERSION_MAJOR(socinfo_get_version()) > 1)
			|| ((SOCINFO_VERSION_MAJOR(socinfo_get_version()) == 1)
			&& (SOCINFO_VERSION_MINOR(socinfo_get_version()) >= 3)))
		{
			pr_info("Setting for Turbo mode 800Mhz\r\n");
			/* R/W latency: 4 cycles; */
		l2x0_init(MSM_L2CC_BASE, 0x0006801B, 0xfe000000);
	}
		else
		{
			pr_info("Setting for non-Turbo mode 600Mhz\r\n");
...
#ifdef	CONFIG_MACH_GT2	
MACHINE_START(MSM7X27_SURF, "SGH-T589 Board")
#ifdef CONFIG_MSM_DEBUG_UART
	.phys_io        = MSM_DEBUG_UART_PHYS,
	.io_pg_offst    = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif
